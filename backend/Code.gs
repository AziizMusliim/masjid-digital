...
function handleRequest(e, method) {
  try {
    ...
    const publicActions = ['getPrayerTimes', 'getAnnouncements', 'getEvents', 'getMedia', 'getDisplaySettings', 'getMasjid', 'testConnection'];
...
function getPrayerTimes(e) {
  ...
  if (!masjid) {
    ...
    return jsonResponse({ status: 'success', data: getDefaultPrayerTimes() });
  }
  ...
  const prayerTimes = calculatePrayerTimes(
    masjid.latitude || -6.2088,
    masjid.longitude || 106.8456,
    masjid.timezone || 'Asia/Jakarta',
    config
  );
...
function getDisplaySettings(e) {
  ...
  if (!settings) {
    ...
    return jsonResponse({ 
      status: 'success', 
      data: {
        theme_color_primary: '#00563F',
        theme_color_secondary: '#DAA520',
        font_family_main: 'Roboto',
        logo_image_url: '',
        footer_text: 'Semoga Allah memberkahi kita semua.'
      }
    });
  }
...
function addAdminUser(email) {
  ...
  for (let i = 1; i < usersData.length; i++) {
    if (usersData[i][headers.indexOf('email')] === email) {
      return 'User already exists with ID: ' + usersData[i][headers.indexOf('id')];
    }
  }
  ...
  const userId = generateId();
  const now = new Date();
  usersSheet.appendRow([userId, email, true, now, now]);
  
  // Add to first masjid with Super Admin role
  const masjidsSheet = ss.getSheetByName('Masjids');
  const masjidsData = masjidsSheet.getDataRange().getValues();
  
  if (masjidsData.length > 1) {
    const masjidId = masjidsData[1][0]; // First masjid
    const rolesSheet = ss.getSheetByName('Roles');
    const rolesData = rolesSheet.getDataRange().getValues();
    const superAdminRoleId = rolesData[1][0]; // Super Admin role
    
    const masjidUsersSheet = ss.getSheetByName('MasjidUsers');
    masjidUsersSheet.appendRow([generateId(), userId, masjidId, superAdminRoleId, now, now]);
  }
  
  return 'Admin user created with ID: ' + userId;
}

// Quick setup function - run this with your email
function setupFirstAdmin() {
  const email = Session.getActiveUser().getEmail();
  if (email) {
    return addAdminUser(email);
  }
  return 'Could not get email. Please run addAdminUser("your-email@example.com") instead.';
}
...
function testConnection() {
  return jsonResponse({
    ...
    driveFolderId: DRIVE_FOLDER_ID ? 'Configured' : 'NOT CONFIGURED'
  });
}
